 ...
 * =====================================================================================
 *
 *       Filename:  magnetic_field.ino
 *
 *    Description:  Demo looks at the 3 axis magnetic field readings from the LSM303 
 *                  magnetometer and outputs them at at a configurable frequency in a
...
void setup() {
  Serial.begin(9600);
  Serial.println("Ardusat Magnetometer Sensor Demo"); 
...
void loop() {
  ...
  mag.getEvent(&event);
  write_json_value("magnetometerX", "m/s^2", event.magnetic.x);
  write_json_value("magnetometerY", "m/s^2", event.magnetic.y);
  write_json_value("magnetometerZ", "m/s^2", event.magnetic.z);
